...
export class PhysicsEngine {
  ...
  updateProjectiles(gs, dt) {
    for (const proj of gs.projectiles) {
      ...
      if (proj.type === 'player_projectile' && gs.boss && gs.boss.alive) {
      ...
      if (proj.type === 'boss_projectile') {
        for (const player of gs.getAlivePlayers()) {
          ...
          if (dist < proj.radius + player.radius) {
            const actual = player.takeDamage(proj.damage || 100, gs.boss || null);
            if (actual > 0) {
              gs.addEvent({
                type: 'damage',
                source: gs.boss ? gs.boss.id : 'boss',
                target: player.id,
                amount: actual,
                skill: 'Projectile',
              });
  ...
  updateAoeZones(gs, dt) {
    for (const zone of gs.aoeZones) {
        ...
        if (zone._tickTimer <= 0) {
          ...
          for (const player of gs.getAlivePlayers()) {
            if (this._isInLaser(player, zone)) {
              const actual = player.takeDamage(zone.damagePerTick, gs.boss || null);
              if (actual > 0) {
                gs.addEvent({
                  type: 'damage',
                  source: gs.boss ? gs.boss.id : 'boss',
      ...
      // Boss persistent AoE zones (lingering damage from boss patterns)
      if (zone.type === 'boss_persistent' && zone._damage) {
        zone._tickTimer = (zone._tickTimer || 0) + dt;
        if (zone._tickTimer >= (zone._tickInterval || 1.0)) {
          zone._tickTimer -= zone._tickInterval || 1.0;
          for (const player of gs.getAlivePlayers()) {
            if (player.dodging) continue;
            const dist = Math.hypot(player.x - zone.x, player.y - zone.y);
            if (dist < zone.radius + player.radius) {
              const actual = zone._isTrueDamage
                ? player.takeTrueDamage(zone._damage)
                : player.takeDamage(zone._damage, gs.boss);
              if (actual > 0) {
                gs.addEvent({
                  type: 'damage',
                  source: zone._bossId || 'boss',
                  target: player.id,
                  amount: actual,
                  skill: 'Zone persistante',
                });
              }
            }
          }
      ...
      // Soul drain zone — drains HP from all hunters, heals boss
      if (zone.type === 'boss_persistent' && zone._isDrain) {
        zone._tickTimer = (zone._tickTimer || 0) + dt;
        if (zone._tickTimer >= (zone._tickInterval || 0.5)) {
          zone._tickTimer -= zone._tickInterval || 0.5;
          const drainPct = zone._drainPct || 3;
          const healMult = zone._healMult || 2;
          let totalDrained = 0;
          for (const player of gs.getAlivePlayers()) {
            const drain = Math.floor(player.maxHp * drainPct / 100);
            const actual = player.takeDamage(drain, gs.boss);
            if (actual > 0) {
              totalDrained += actual;
              gs.addEvent({ type: 'damage', source: zone._bossId || 'boss', target: player.id, amount: actual, skill: 'Drain d\'Âme' });
            }
          }
          // Boss heals from drained HP
          if (totalDrained > 0 && gs.boss && gs.boss.alive) {
            const healAmount = Math.floor(totalDrained * healMult);
            gs.boss.hp = Math.min(gs.boss.maxHp, gs.boss.hp + healAmount);
            gs.addEvent({ type: 'boss_heal', source: gs.boss.id, amount: healAmount });
          }
        }
      }
      ...
      if (zone.type === 'fire_wave' && zone.active && zone.damagePerTick) {
        ...
        if (zone._expandRate) {
          zone.radius = Math.min(zone._maxRadius || 500, zone.radius + zone._expandRate * dt);
        }
        zone._tickTimer = (zone._tickTimer || 0) - dt;
        if (zone._tickTimer <= 0) {
          zone._tickTimer = zone.tickInterval || 0.15;

          // Damage hunters near the ring edge (not inside — it's a wave)
          const ringWidth = 60;
          for (const player of gs.getAlivePlayers()) {
            if (player.dodging) continue;
            const dist = Math.hypot(player.x - zone.x, player.y - zone.y);
            if (dist > zone.radius - ringWidth && dist < zone.radius + player.radius) {
              const actual = player.takeDamage(zone.damagePerTick, gs.boss || null);
              if (actual > 0) {
                gs.addEvent({
                  type: 'damage',
