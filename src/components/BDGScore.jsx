// src/components/BDGScore.jsx
import React, { useState, useEffect } from 'react';
import { useTranslation } from 'react-i18next';
import BDGViewMode from './ScoreCard/BDGViewMode';
import BDGEditMode from './ScoreCard/BDGEditMode';
import { validateBuildCompleteness } from './ScoreCard/bdgService';
import '../i18n/i18n';
...
const BDGScore = () => {
  ...
  const [tankMessages, setTankMessages] = useState([]);
  
  // Récupérer activeAccount depuis localStorage
  const [activeAccount] = useState(() => {
    try {
      const data = JSON.parse(localStorage.getItem('builderberu_users'));
      const username = Object.keys(data)[0];
      return data[username]?.activeAccount || 'main';
    } catch {
      return 'main';
    }
  });

  // Fonction showTankMessage locale
  const showTankMessage = (message, priority = false, entityType = 'beru') => {
    const newMessage = {
      id: Date.now(),
      message,
      priority,
    ...
    setTankMessages(prev => [...prev, newMessage]);
    
    // Auto-clear après 5 secondes
    setTimeout(() => {
      setTankMessages(prev => prev.filter(m => m.id !== newMessage.id));
    }, 5000);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-4">
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {tankMessages.map(msg => (
          <div key={msg.id} className="bg-purple-900/90 text-white px-4 py-2 rounded-lg shadow-lg animate-fade-in">
            {msg.message}
          </div>
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-indigo-700 to-purple-800 rounded-t-xl p-3 sm:p-4">
          {/* Desktop Header */}
        ...
        <div className="bg-gray-900 rounded-b-xl p-3 sm:p-6">
          {(() => {
        ...
        <div className="mt-4 bg-gray-800 rounded-xl p-3 sm:p-4 flex flex-col sm:flex-row justify-between items-center gap-3">
