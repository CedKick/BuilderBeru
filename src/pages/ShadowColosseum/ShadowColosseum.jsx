...
const BERU_SHORTCUT_PHRASES = {
  chibis: [
    ...
    { msg: "Les Chibis c'est la vie. Beru a dit.", mood: 'happy' },
    { msg: "Qui a nourri les Chibis aujourd'hui ? ...Personne ? SCANDALE.", mood: 'shocked' },
    { msg: "Beru veut un Chibi de lui-meme. C'est trop demander ?", mood: 'thinking' },
    { msg: "Petit rappel : les Chibis jugent tes choix de build.", mood: 'normal' },
    { msg: "Un Chibi 5 etoiles ca change une vie. Enfin presque.", mood: 'excited' },
    { msg: "Les Chibis te saluent ! ...Enfin, ceux qui t'aiment bien.", mood: 'happy' },
  ],
  hunters: [
    ...
    { msg: "Ton roster est... interessant. Beru est diplomate.", mood: 'normal' },
    { msg: "50 hunters et toujours personne pour tank correctement.", mood: 'angry' },
    { msg: "Beru inspecte les troupes. Garde-a-vous !", mood: 'excited' },
    { msg: "Y'a du potentiel la-dedans. Faut juste... beaucoup de farm.", mood: 'thinking' },
    { msg: "Tes hunters ont confiance en toi. Sauf celui du fond.", mood: 'happy' },
    { msg: "Direction la caserne ! Beru veut compter les etoiles.", mood: 'happy' },
  ],
  arc: [
    ...
    { msg: "Beru a un mauvais pressentiment sur ce donjon...", mood: 'thinking' },
    { msg: "T'as assez de potions ? Non ? On y va quand meme.", mood: 'excited' },
    { msg: "Les Donjons ARC attendent personne. FONCE.", mood: 'angry' },
    { msg: "Beru se porte volontaire pour rester a l'entree.", mood: 'happy' },
    { msg: "Un donjon par jour eloigne le medecin pour toujours. Ou pas.", mood: 'thinking' },
    { msg: "C'est parti pour le loot ! ...ou le wipe. Suspense.", mood: 'excited' },
  ],
  troll: [
    ...
    { msg: "Bienvenue dans l'ascenseur panoramique de Beru. Vue : magnifique.", mood: 'happy' },
    { msg: "Scroll ralenti par decret royal de Beru. Pas d'appel possible.", mood: 'excited' },
    { msg: "Tu voulais scroller ? Beru veut que tu APPRECIES le chemin.", mood: 'normal' },
    { msg: "Beru a active le mode zen. Respire. Expire. Attends.", mood: 'thinking' },
    { msg: "5 secondes de scroll. 5 secondes de reflexion sur tes choix.", mood: 'happy' },
    { msg: "Beru fait du syndicalisme : pause obligatoire entre les clics.", mood: 'excited' },
    { msg: "Ralentissement strategique. Beru appelle ca de la pedagogie.", mood: 'thinking' },
    { msg: "La lenteur est une vertu. Beru est tres vertueux.", mood: 'happy' },
    { msg: "T'as voulu jouer au plus malin avec Beru ? Erreur.", mood: 'angry' },
    { msg: "Beru te punit, mais avec amour. Enfin... surtout de la malice.", mood: 'happy' },
  ],
  annoyed: [
    ...
    { msg: "Beru a commence a compter tes clics. T'es a combien la ? TROP.", mood: 'angry' },
    { msg: "Tu crois que c'est un jeu ? ...Ah oui c'en est un. MAIS QUAND MEME.", mood: 'shocked' },
    { msg: "Beru envisage serieusement de se mettre en greve.", mood: 'thinking' },
    { msg: "Encore un clic et Beru appelle les RH.", mood: 'angry' },
    { msg: "Tu cliques. Beru souffre. C'est ca ton kiff ?", mood: 'shocked' },
    { msg: "Beru va ecrire un rapport. Avec ton nom dessus.", mood: 'normal' },
    { msg: "Les boutons ont des sentiments. Beru aussi. ARRETE.", mood: 'angry' },
    { msg: "Beru note cet incident dans ton dossier permanent.", mood: 'thinking' },
    { msg: "C'est un ascenseur, pas un trampoline !", mood: 'shocked' },
    { msg: "Beru va mettre un CAPTCHA sur ces boutons, promis.", mood: 'angry' },
  ],
};
      ...
      fetch(`/api/storage/load-raid?username=${encodeURIComponent(authUser.username)}`)
        ...
        .then(d => {
          if (d.success && d.raidData) {
            ...
            // Sync expedition data into shadow_colosseum_raid localStorage for Codex
            if (d.raidData.expeditionInventory || d.raidData.expeditionCurrencies || d.raidData.expeditionEssences) {
              try {
                const raidLocal = JSON.parse(localStorage.getItem('shadow_colosseum_raid') || '{}');
                if (d.raidData.expeditionInventory) raidLocal.expeditionInventory = d.raidData.expeditionInventory;
                if (d.raidData.expeditionCurrencies) raidLocal.expeditionCurrencies = d.raidData.expeditionCurrencies;
                if (d.raidData.expeditionEssences) raidLocal.expeditionEssences = d.raidData.expeditionEssences;
                localStorage.setItem('shadow_colosseum_raid', JSON.stringify(raidLocal));
              } catch {}
            }
    ...
    <div className="min-h-screen bg-[#0a0a1a] text-white pb-20">
      ...
      {view === 'equipment' && manageTarget && (() => {
        ...
        return (
          <div className="max-w-2xl mx-auto px-4 pt-4">
            ...
            {(() => {
              const expInv = raidStorage?.expeditionInventory || [];
              if (expInv.length === 0) return null;

              // Slot mapping: expedition (EN) → colosseum (FR)
              const EXP_SLOT_MAP = { helm: 'casque', chest: 'plastron', gloves: 'gants', boots: 'bottes' };
              const EXP_SLOT_ICONS = { helm: '\u26D1\uFE0F', chest: '\uD83E\uDDBA', gloves: '\uD83E\uDDE4', boots: '\uD83D\uDC62', weapon: '\u2694\uFE0F' };
              const EXP_RARITY_COLORS = {
                common: { color: 'text-gray-400', bg: 'bg-gray-700/20', border: 'border-gray-600/30' },
                uncommon: { color: 'text-green-400', bg: 'bg-green-900/20', border: 'border-green-600/30' },
                rare: { color: 'text-blue-400', bg: 'bg-blue-900/20', border: 'border-blue-600/30' },
                epic: { color: 'text-purple-400', bg: 'bg-purple-900/20', border: 'border-purple-600/30' },
                legendary: { color: 'text-amber-400', bg: 'bg-amber-900/20', border: 'border-amber-500/30' },
                mythique: { color: 'text-red-400', bg: 'bg-red-900/20', border: 'border-red-500/30' },
              };
              const EXP_STAT_NAMES = {
                hp_flat: 'PV', atk_flat: 'ATK', def_flat: 'DEF', spd_flat: 'SPD',
                crit_rate: 'CRIT%', res_flat: 'RES', hp_pct: 'PV%', atk_pct: 'ATK%',
                def_pct: 'DEF%', crit_dmg: 'CRIT DMG',
              };
              const EXP_STAT_COLORS = {
                hp_flat: 'text-green-400', atk_flat: 'text-red-400', def_flat: 'text-blue-400',
                spd_flat: 'text-emerald-400', crit_rate: 'text-yellow-400', res_flat: 'text-cyan-400',
                hp_pct: 'text-green-400', atk_pct: 'text-red-400', def_pct: 'text-blue-400',
                crit_dmg: 'text-yellow-400',
              };

              // Filter: only armor items (equippable to current hunter)
              const equipableItems = expInv.filter(item => item.type === 'armor' || item.type === 'set_piece');
              const weaponItems = expInv.filter(item => item.type === 'weapon');

              // Equip expedition item: convert to forge-compatible format
              const equipExpItem = (item) => {
                const mappedSlot = EXP_SLOT_MAP[item.slot];
                if (!mappedSlot) return; // weapon or unknown slot — skip for now

                // Convert expedition stats to forge artifact format
                ...
                // Remove from expedition inventory in raidStorage
                setRaidStorage(prev => ({
                  ...prev,
                  expeditionInventory: (prev?.expeditionInventory || []).filter(i => i.uid !== item.uid),
                }));
              };

              return (
                <div className="mt-3 pt-3 border-t border-amber-500/20">
                  <div className="text-normal-responsive font-bold text-amber-400 mb-2 flex items-center gap-1.5">
                    <span>{'\u2694\uFE0F'}</span> Butin d'Expedition ({expInv.length})
                  </div>

                  {/* Armor items */}
                  {equipableItems.length > 0 && (
                    <div className="mb-2">
                      <div className="text-small-responsive text-gray-500 mb-1">{'\uD83D\uDEE1\uFE0F'} Armures ({equipableItems.length})</div>
                      <div className="grid grid-cols-2 gap-1.5 max-h-48 overflow-y-auto">
                        {equipableItems.map((item) => {
                          const rc = EXP_RARITY_COLORS[item.rarity] || EXP_RARITY_COLORS.common;
                          return (
                              ...
                              {item.stats && (
                                <div className="mt-1 pt-1 border-t border-gray-700/20 space-y-px">
                                  {Object.entries(item.stats).map(([statId, val]) => (
                                    <div key={statId} className={`text-small-responsive ${EXP_STAT_COLORS[statId] || 'text-gray-400'}`}>
                                      {EXP_STAT_NAMES[statId] || statId} +{val}
                                    </div>
                                  ))}
                                </div>
                              )}
                  ...
                  {weaponItems.length > 0 && (
                    <div className="mb-2">
                      <div className="text-small-responsive text-gray-500 mb-1">{'\u2694\uFE0F'} Armes ({weaponItems.length})</div>
                      <div className="grid grid-cols-2 gap-1.5 max-h-48 overflow-y-auto">
                        {weaponItems.map((item) => {
                          const rc = EXP_RARITY_COLORS[item.rarity] || EXP_RARITY_COLORS.common;
                          return (
                            <div key={item.uid}
                              className={`p-2 rounded-lg border ${rc.border} ${rc.bg} text-left`}>
                              <div className="flex items-center gap-1">
                              ...
                              {item.description && (
                                <div className="text-tiny-responsive text-orange-300/60 mt-0.5 italic">{item.description}</div>
                              )}
                            </div>
                          );
                        })}
      ...
      {view === 'hub' && createPortal(
        <div style={{ position: 'fixed', right: 'calc(50% - 380px)', top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, zIndex: 99999, pointerEvents: 'auto' }}>
          ...
          {/* Chibi shortcut */}
          <button
            onClick={() => { const el = document.getElementById('section-chibis'); if (el) { _shortcutClickCount++; clearTimeout(_shortcutResetTimer); _shortcutResetTimer = setTimeout(() => { _shortcutClickCount = 0; }, 20000); const isTroll = _shortcutClickCount >= 3 && Math.random() < 0.6; const rect = el.getBoundingClientRect(); elevatorScroll(window.scrollY + rect.top - window.innerHeight / 2 + rect.height / 2, 900, isTroll); const p = isTroll ? randomPick(BERU_SHORTCUT_PHRASES.troll) : _shortcutClickCount >= 3 ? randomPick(BERU_SHORTCUT_PHRASES.annoyed) : randomPick(BERU_SHORTCUT_PHRASES.chibis); beruSay(p.msg, p.mood); } }}
            title="Chibis"
          ...
          {/* Hunter shortcut */}
          <button
            onClick={() => { const el = document.getElementById('section-hunters'); if (el) { _shortcutClickCount++; clearTimeout(_shortcutResetTimer); _shortcutResetTimer = setTimeout(() => { _shortcutClickCount = 0; }, 20000); const isTroll = _shortcutClickCount >= 3 && Math.random() < 0.6; const rect = el.getBoundingClientRect(); elevatorScroll(window.scrollY + rect.top - window.innerHeight / 2 + rect.height / 2, 900, isTroll); const p = isTroll ? randomPick(BERU_SHORTCUT_PHRASES.troll) : _shortcutClickCount >= 3 ? randomPick(BERU_SHORTCUT_PHRASES.annoyed) : randomPick(BERU_SHORTCUT_PHRASES.hunters); beruSay(p.msg, p.mood); } }}
            title="Hunters"
            style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, cursor: 'pointer', transition: 'all 0.2s', padding: 0, fontSize: 16 }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.15)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.4)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; }}
          ...
          <button
            onClick={() => { const el = document.getElementById('section-arc'); if (el) { _shortcutClickCount++; clearTimeout(_shortcutResetTimer); _shortcutResetTimer = setTimeout(() => { _shortcutClickCount = 0; }, 20000); const isTroll = _shortcutClickCount >= 3 && Math.random() < 0.6; const rect = el.getBoundingClientRect(); elevatorScroll(window.scrollY + rect.top - window.innerHeight / 2 + rect.height / 2, 900, isTroll); const p = isTroll ? randomPick(BERU_SHORTCUT_PHRASES.troll) : _shortcutClickCount >= 3 ? randomPick(BERU_SHORTCUT_PHRASES.annoyed) : randomPick(BERU_SHORTCUT_PHRASES.arc); beruSay(p.msg, p.mood); } }}
            title="Donjons ARC"
            style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, cursor: 'pointer', transition: 'all 0.2s', padding: 0, fontSize: 16 }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.15)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.4)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; }}
