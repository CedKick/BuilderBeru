...
const RaidChibi = React.memo(function RaidChibi({ chibi, isFlashing, isHit, isHealing }) {
  return (
    <div className={`relative flex items-center gap-2 p-1.5 rounded-lg transition-all ${
      !chibi.alive ? 'opacity-30' : ''
    }`}
    style={{
      background: isHealing ? 'rgba(34,197,94,0.2)' : isHit ? 'rgba(239,68,68,0.25)' : isFlashing ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.03)',
      animation: !chibi.alive ? 'koFade 0.5s ease-out forwards' :
        isHealing ? 'healSparkle 0.5s ease-out' :
        isFlashing ? 'raidChibiFlash 0.3s ease-out' :
        isHit ? 'hitShake 0.3s ease-out' : 'none',
    }}>
      ...
      {isHealing && chibi.alive && (
        <div className="absolute inset-0 pointer-events-none rounded-lg overflow-hidden" style={{
          ...
          animation: 'healSparkle 0.5s ease-out forwards',
        }} />
      )}
      <img src={chibi.sprite} alt={chibi.name}
        className={`w-9 h-9 rounded-full object-cover border-2 ${
          !chibi.alive ? 'grayscale border-gray-700' : 'border-white/20'
        }`}
        style={{
          filter: !chibi.alive ? 'grayscale(1) brightness(0.4)' :
            chibi.rarity ? (RARITY[chibi.rarity]?.glow || '') : '',
        }} />
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-bold truncate">{chibi.name}</span>
          <span className={`text-[9px] ${!chibi.alive ? 'text-red-500 font-bold' : 'text-gray-400'}`}>
            {!chibi.alive ? 'K.O.' : `${Math.round(chibi.hp / chibi.maxHp * 100)}%`}
          </span>
        </div>
        <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-200 ${
            chibi.hp / chibi.maxHp > 0.5 ? 'bg-green-500' : chibi.hp / chibi.maxHp > 0.2 ? 'bg-yellow-500' : 'bg-red-500'
          }`} style={{ width: `${chibi.maxHp > 0 ? (chibi.hp / chibi.maxHp * 100) : 0}%` }} />
        </div>
      </div>
    </div>
  );
...
export function RaidArena({ battleState, vfxQueue, timer, isPaused, sungCooldowns, activeSungBuffs, combatLog, onTogglePause, onSungSkill, phase }) {
  ...
  const chibiFlashing = useMemo(() => {
    const set = new Set();
    recentVFX.filter(v => v.type === 'chibi_attack' && now - v.timestamp < 300).forEach(v => set.add(v.sourceId));
    return set;
  }, [recentVFX, now]);
  const chibiHit = useMemo(() => {
    const set = new Set();
    recentVFX.filter(v => v.type === 'boss_attack' && now - v.timestamp < 400).forEach(v => set.add(v.targetId));
    return set;
  }, [recentVFX, now]);
  ...
  const healNumbers = recentVFX.filter(v => v.type === 'heal' && v.value > 0 && now - v.timestamp < 700);

  // Floating damage numbers from queue
  const dmgNumbers = recentVFX.filter(v => (v.type === 'chibi_attack' || v.type === 'boss_attack') && v.damage > 0 && now - v.timestamp < 700);
  ...
  return (
    <div className="space-y-3 relative">
      ...
      {/* ─── BOSS HEADER ─── */}
      <div className="relative bg-gradient-to-r from-red-900/40 to-amber-900/40 border border-red-500/30 rounded-xl p-3 overflow-hidden"
        ...
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <span className="text-2xl" style={{ animation: bossHit ? 'hitShake 0.3s ease-out' : 'idleBreathe 3s ease-in-out infinite' }}>{b.emoji}</span>
            <span className="font-bold text-red-400">{b.name}</span>
            {b.phase && <span className="text-xs px-2 py-0.5 rounded bg-red-500/20 text-red-300">{b.phase.name}</span>}
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-400">RC: {b.barsDestroyed}</span>
            ...
            <button onClick={onTogglePause} className="text-xs px-2 py-1 rounded bg-white/10 hover:bg-white/20">
              {isPaused ? 'Play' : 'Pause'}
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-1 mb-1 justify-center">
          {Array.from({ length: b.totalBars }).map((_, i) => (
            <div key={i} className={`w-3 h-3 rotate-45 border transition-all ${
        ...
        <div className="absolute top-0 left-0 right-0 pointer-events-none">
          {dmgNumbers.filter(v => v.type === 'chibi_attack').slice(-4).map(v => (
            <div key={v.id} className="absolute whitespace-nowrap"
              style={{
                left: `${30 + Math.random() * 40}%`,
                top: 8,
                color: v.isCrit ? '#fbbf24' : ELEMENT_COLORS_RAW[v.element] || '#fff',
                fontWeight: 900,
                fontSize: v.isCrit ? '0.9rem' : '0.75rem',
      ...
      <div className="grid grid-cols-2 gap-3">
        {[0, 1].map(teamIdx => {
          const teamChibis = chibis.slice(teamIdx * 3, teamIdx * 3 + 3);
          return (
            <div key={teamIdx} className="bg-white/5 rounded-xl p-2 border border-white/10">
              <div className="text-[10px] text-center text-gray-500 mb-1">Team {teamIdx + 1}</div>
              <div className="space-y-1.5">
                {teamChibis.map(c => (
                  <RaidChibi key={c.id} chibi={c}
                    isFlashing={chibiFlashing.has(c.id)}
      ...
      {/* ─── SUNG JINWOO SKILLS ─── */}
      <div className="bg-gradient-to-r from-purple-900/30 to-indigo-900/30 border border-purple-500/20 rounded-xl p-3">
        <div className="text-xs text-center text-purple-300 mb-2 font-bold">Sung Jinwoo — Skills</div>
          ...
          {SUNG_SKILLS.map(skill => {
            ...
            return (
              <button key={skill.id}
                ...
                disabled={onCD}
                className={`relative flex flex-col items-center p-2 rounded-lg border transition-all min-w-[60px] ${
                ...
                <span className="text-[9px] text-white/80">{skill.name}</span>
                <span className="text-[8px] text-gray-400">[{skill.key.toUpperCase()}{skill.altKey ? `/${skill.altKey.toUpperCase()}` : ''}]</span>
                {onCD && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-lg">
                    <span className="text-sm font-bold text-red-400">{cdRemain}s</span>
                  </div>
                )}
                {isActive && (
      ...
      {/* ─── COMBAT LOG ─── */}
      <div className="bg-black/40 rounded-xl p-2 border border-white/5 max-h-24 overflow-y-auto text-[10px] font-mono">
