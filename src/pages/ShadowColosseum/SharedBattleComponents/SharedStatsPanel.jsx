// src/pages/ShadowColosseum/SharedBattleComponents/SharedStatsPanel.jsx
// Shared detailed stats panel that appears when clicking on a fighter or boss
...
export default function SharedStatsPanel({ entity, weapon = null, logs, onClose }) {
  const isBoss = entity.id === 'training_dummy' || entity.id?.includes('boss') || entity.id === 'ant_queen';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm text-gray-400">Lv.{entity.level || '?'}</p>
              </div>
              {weapon && (
            ...
            <div className="space-y-2">
              {entity.buffs.map((buff, idx) => (
                <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-green-600/10 border border-green-500/20">
                  ...
                  {buff.turns && <span className="text-xs text-gray-400">{buff.turns} tours</span>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {entity.passiveState.katanaZStacks !== undefined && (
                <div className="p-2 rounded-lg bg-red-600/10 border border-red-500/20">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-white">Tranchant Éternel (Katana Z)</span>
              ...
              {entity.passiveState.katanaVState && (
                <div className="p-2 rounded-lg bg-purple-600/10 border border-purple-500/20">
                  <div className="text-sm font-bold text-white mb-1">Bénédiction Divine (Katana V)</div>
              ...
              {entity.passiveState.flammeStacks !== undefined && entity.passiveState.flammeStacks > 0 && (
                <div className="p-2 rounded-lg bg-orange-600/10 border border-orange-500/20">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-white">Inner Flame</span>
                    <span className="text-sm text-orange-300">x{entity.passiveState.flammeStacks} stacks</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-1">+{entity.passiveState.flammeStacks * 3}% DMG</p>
                </div>
              )}
            </div>
          </div>
